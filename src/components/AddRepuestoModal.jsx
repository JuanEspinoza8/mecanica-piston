...
export default function AddRepuestoModal({ isOpen, onClose, ordenId }) {
  ...
  const onSubmit = async (data) => {
    try {
      const newRepuesto = await addRepuesto({
        ordenId,
        nombre: data.nombre,
        costo: data.precio,
      ...
      if (selectedFile) {
        ...
        await uploadFile({ file: selectedFile, ordenId, tipo, repuestoId: newRepuesto.id });
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  inputMode="numeric"
                  value={watchCantidad ? watchCantidad.toLocaleString('es-AR') : ''}
                  onChange={(e) => {
                    const raw = e.target.value.replace(/\D/g, '');
                    setValue('cantidad', raw ? parseInt(raw, 10) : 0, { shouldValidate: true });
                  }}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  inputMode="numeric"
                  value={watchPrecio ? watchPrecio.toLocaleString('es-AR') : ''}
                  onChange={(e) => {
                    const raw = e.target.value.replace(/\D/g, '');
                    setValue('precio', raw ? parseInt(raw, 10) : 0, { shouldValidate: true });
                  }}
                  onFocus={(e) => e.target.select()}
